...
func TestDb(t *testing.T) {
	...
	manager, err := Init("", conf)
	assert.Nil(err)
	defer func() {
		_ = syscall.Unlink("abc")
	}()

	defer func() {
		err := manager.CloseAll()
		assert.Nil(err)
		_, err = manager.Database("unit_test")
		assert.NotNil(err)
	}()
	...
	tableSql := "CREATE TABLE `good_unit_test` (" +
		...
		"`uid` bigint(20) NOT NULL UNIQUE DEFAULT 0," +
	...
	db, err := manager.Database("unit_test")
	assert.Nil(err)
	...
	n, err = db.Table(tableName).Where("uid", "=", user.Uid).UpdateRaw(strSet)
	if err != nil {
		assert.Nil(err, err.Error())
	}
	assert.Equal(n, int64(1))
...
func CheckTable(db *DB, table string) (bool, error) {
	descTable := `select * from sqlite_master where type="table" and name="` + table + `";`
